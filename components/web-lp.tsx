...
const COMPARE = [
  { label: "初期費用", makers: "数十万円〜", free: "数万円〜", diy: "無料〜", plus: "抑えめ・要相談", plusGood: true },
  { label: "月額・運用", makers: "保守は別料金", free: "個人対応・不安定", diy: "数千円（自力運用）", plus: "5,000円〜／月", plusGood: true },
  { label: "納期", makers: "1〜3ヶ月", free: "ばらつきあり", diy: "自分次第", plus: "最短1週間", plusGood: true },
  { label: "デザイン品質", makers: "高い", free: "人による", diy: "テンプレ任せ", plus: "プロが対応", plusGood: true },
  { label: "公開後の更新", makers: "都度費用", free: "対応まちまち", diy: "すべて自分で", plus: "月額に込み／代行可", plusGood: true },
  { label: "集客・SEO/MEO", makers: "別途依頼", free: "対応外が多い", diy: "自力", plus: "対応可", plusGood: true },
  { label: "窓口", makers: "担当制", free: "個人", diy: "なし", plus: "一本化", plusGood: true },
]
    ...
    <div className="weblp min-h-screen bg-white pb-20 text-slate-800 antialiased lg:pb-0">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-100 bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4">
          ...
          <Link href="/contact" className="rounded-full px-5 py-2 text-sm font-bold text-white shadow-sm" style={{ background: CTA }}>
            無料で相談する
          </Link>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-blue-50 via-white to-white">
        <div className="mx-auto max-w-5xl px-4 pt-12 pb-14 text-center lg:pt-16">
          ...
          <h1 className="mx-auto mt-6 max-w-3xl text-3xl font-extrabold leading-[1.35] tracking-tight text-slate-900 sm:text-4xl lg:text-[2.9rem]">
          ...
          <div className="mx-auto mt-10 grid max-w-2xl grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <div key={s.n} className="rounded-xl border border-slate-200 bg-white px-3 py-4">
      ...
      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-5xl px-4">
          <h2 className="text-center text-xl font-extrabold text-slate-900 sm:text-2xl">こんなお悩み、ありませんか？</h2>
        ...
        <div className="mx-auto max-w-5xl px-4">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {FEATURES.map((f, i) => {
              const Icon = f.icon
              return (
                <div key={f.t} className="rounded-2xl border border-slate-200 bg-white p-7 text-center">
      ...
      <section className="py-12" style={{ background: BLUE }}>
        <div className="mx-auto max-w-3xl px-4 text-center">
          ...
          <Link href="/contact" className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-8 py-4 text-base font-bold text-slate-900 shadow-lg transition-transform hover:-translate-y-0.5" style={{ color: CTA }}>
            無料で相談する <ArrowRight className="h-5 w-5" />
          </Link>
        </div>
      </section>
      ...
      <section className="py-16 lg:py-20">
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {PRICING.map((p, i) => (
              <div key={p.nm} className={`relative rounded-2xl border bg-white p-7 ${i === 0 ? "border-2 shadow-lg" : "border-slate-200"}`} style={i === 0 ? { borderColor: BLUE } : undefined}>
                {i === 0 && (
        ...
        <div className="mx-auto max-w-5xl px-4">
          ...
          {works.length > 0 ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {works.map((w) => (
                <Link key={w.id} href={`/works?id=${w.id}`} className="group overflow-hidden rounded-xl border border-slate-200 bg-white transition-shadow hover:shadow-lg">
...
function SecHead({ eyebrow, title, sub }: { eyebrow: string; title: string; sub?: string }) {
  return (
    <div className="mb-10 text-center">
      <p className="text-xs font-bold tracking-[0.2em]" style={{ color: CTA }}>{eyebrow}</p>
